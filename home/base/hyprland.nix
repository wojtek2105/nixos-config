...
let
  ...
  mic-mute-control = pkgs.writeShellApplication {
    ...
    text = ''
      # SwayOSD's mute action shows an OSD but provides no reliable result to
      # Hyprland. Toggle PipeWire directly, then display the state we read
      # back so the M3 key always reports the microphone's real mute status.
      if ! wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle; then
        notify-send --urgency=critical "Mikrofon" "Nie udało się przełączyć wyciszenia źródła PipeWire."
        exit 1
      fi

      if wpctl get-volume @DEFAULT_AUDIO_SOURCE@ | ${pkgs.gnugrep}/bin/grep -q '\[MUTED\]'; then
        swayosd-client \
          --custom-message="Mikrofon wyciszony" \
          --custom-icon=audio-input-microphone-muted-symbolic
      else
        swayosd-client \
          --custom-message="Mikrofon włączony" \
...
in
{
  xdg.configFile."uwsm/env".text = ''
    # Greetd starts UWSM directly, outside a login shell. Keep the Home Manager
    # and system application directories visible to systemd/DBus services so
    # portals can resolve MIME handlers such as swayimg.desktop.
    export XDG_DATA_DIRS="/etc/profiles/per-user/${username}/share:/run/current-system/sw/share:/nix/var/nix/profiles/default/share''${XDG_DATA_DIRS:+:$XDG_DATA_DIRS}"
  '';
    ...
    extraLuaFiles."config" = (builtins.replaceStrings
      ...
      [
        ...
        (lib.optionalString laptopEnabled ''
          ...
          bind_exec("XF86AudioMicMute", "mic-mute-control", { locked = true })
  ...
  programs.hyprlock = {
    ...
    settings = {
      ...
      image = {
        ...
        # 372 px is three times the prior lock-screen size.  Keep the raster
        # at 660 px so Hyprlock never has to upscale it on dense displays.
        size = 372;
        # Hyprlock's default -1 clips images into a circle; zero preserves the
        # transparent, full Tux + TOTEM silhouette without a border.
        rounding = 0;
        border_size = 0;
        position = "0, 320";
        halign = "center";
