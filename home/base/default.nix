...
let
  ...
  captureShortcuts = [
    (shortcut "Print" "Wybierz okno lub obszar; zapisz PNG i skopiuj go do schowka")
    (shortcut "Shift+Print" "Przechwyć aktywne okno, zapisz PNG i skopiuj go do schowka")
    (shortcut "Ctrl+Print" "Przechwyć cały ekran, zapisz PNG i skopiuj go do schowka")
    (shortcut "Super+Shift+S" "Otwórz menu screenshotów i przełącznik edycji Satty")
    (shortcut "Super+Ctrl+S" "Uruchom animowany wygaszacz WOJTECH")
    (shortcut "Satty: Enter" "Zapisz PNG, skopiuj go do schowka i zamknij edytor")
  ...
  ++ lib.optionals laptopEnabled [
    (shortcut "M3" "Przełącz wyciszenie domyślnego mikrofonu PipeWire")
  ...
  screenshot-menu = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      wait_for_selection_overlay() {
        ...
        # dimming layer. One compositor frame keeps it out of Wayshot without
        # making the direct Print shortcuts feel delayed.
        sleep 0.04
      }

      # The persisted switch defaults to direct PNG capture when absent; keep
      # it in XDG state so a chosen Satty mode survives future logins.
      state_directory="''${XDG_STATE_HOME:-$HOME/.local/state}/screenshot-menu"
      edit_mode_file="$state_directory/edit-mode"

      editing_enabled() {
        [[ -r "$edit_mode_file" && "$(<"$edit_mode_file")" == enabled ]]
      }
      ...
      toggle_editing() {
        ...
        if editing_enabled; then
          printf 'disabled\n' > "$edit_mode_file"
          notify-send --app-name=Screenshot --icon=camera-photo-symbolic \
            'Edycja screenshotów' 'Wyłączona: następne zrzuty zapiszą PNG i skopiują go do schowka.'
        else
          printf 'enabled\n' > "$edit_mode_file"
          notify-send --app-name=Screenshot --icon=camera-photo-symbolic \
            'Edycja screenshotów' 'Włączona: następny zrzut otworzy Satty.'
        fi
      }

      open_editor() {
        local geometry="''${1:-}" connector pci_address dri_prime temporary_image satty_error
        local -a satty_args satty_env

        # Pick the GPU that drives the connected internal panel rather than
        # relying on Mesa's ambiguous numeric DRI_PRIME order. Discover it
        # read-only with: readlink -f /sys/class/drm/card*-eDP-*/device
        for connector in /sys/class/drm/card*-eDP-*/status; do
          [[ -r "$connector" && "$(cat "$connector")" == connected ]] || continue
          pci_address="$(basename "$(readlink -f "$(dirname "$connector")/device")")"
          dri_prime="pci-$(printf '%s' "$pci_address" | tr ':.' '_')"
          break
        done
        ...
        # GTK may probe every Vulkan device for dmabuf support even when GSK
        # draws through Cairo. On hybrid AMD laptops that enumeration alone
        # wakes the dGPU. Disable Vulkan discovery and render Satty through
        # OpenGL on the GPU driving the connected eDP panel.
        satty_env=(
          DRI_PRIME="$dri_prime"
          GDK_BACKEND=wayland
          GDK_DISABLE=vulkan
        ...
        # Satty 0.22 cannot decode PPM or QOI. Keep this short-lived PNG only
        # for its input; the normal no-editor path avoids this encoding fully.
        temporary_image="$(mktemp --tmpdir --suffix=.png screenshot.XXXXXX)"
        if [[ -n "$geometry" ]]; then
          env DRI_PRIME="$dri_prime" wayshot \
            -g "$geometry" "$temporary_image"
        else
          env DRI_PRIME="$dri_prime" wayshot \
            "$temporary_image"
        fi || {
          rm -f "$temporary_image"
          notify-send --urgency=critical 'Screenshot' 'Nie udało się przechwycić PNG dla Satty.'
          return 1
        }

        satty_error="''${XDG_RUNTIME_DIR:?}/screenshot-satty-error.log"
        if ! env "''${satty_env[@]}" satty \
          --filename "$temporary_image" "''${satty_args[@]}" 2>"$satty_error"; then
          notify-send --urgency=critical 'Screenshot' \
            "Nie udało się uruchomić edytora Satty. Szczegóły: $satty_error"
          rm -f "$temporary_image"
          return 1
        fi
      ...
      if [[ -z "$mode" ]]; then
        ...
        while true; do
          if editing_enabled; then
            edit_label='☑  Edycja w Satty: włączona'
          else
            edit_label='☐  Edycja w Satty: wyłączona'
          fi
          ...
          choice="$({
            ...
            --prompt 'Zrzut ekranu  ›  ' --width 48 --lines 4)" || exit 0

          case "$choice" in
            *'Edycja w Satty'*) toggle_editing ;;
            *Obszar) mode=area; break ;;
            *'Aktywne okno') mode=window; break ;;
            *'Cały ekran') mode=full; break ;;
            *) exit 0 ;;
          esac
        done
      fi
      ...
      case "$mode" in
        ...
        screen|full|'󰍹  Cały ekran') target=screen ;;
        *) exit 0 ;;
      esac

      # Only the Fuzzel path needs to wait for its surface to unmap. Direct
      # shortcuts and Ironbar clicks bypass this delay entirely.
      if "$opened_menu"; then
        sleep 0.2
      fi
      ...
      capture() {
        ...
        if editing_enabled; then
          open_editor "$geometry"
        elif ! save_without_editor "$geometry"; then
      ...
      while true; do
        ...
        case "$choice" in
          ...
          *"Narzędzia")
            root_selection="$choice"
            menu_result="$({
              ${lib.optionalString voxtypeEnabled "printf '%s\\n' '󰑋  Dyktowanie Voxtype' '󰍹  Stan Voxtype w terminalu' '󰍉  Konfiguruj Voxtype'"}
              printf '%s\n' \
                '󰃀  Historia schowka' \
                '  Wróć'
            } | choose 'Pulpit  ›  Narzędzia' ${if voxtypeEnabled then "5" else "2"} "$tools_selection")" || exit 0
            IFS=$'\t' read -r navigation action <<< "$menu_result"
  ...
  desktop-panel = pkgs.writeShellApplication {
    ...
    runtimeInputs = [
      ...
      pkgs.util-linux
      pkgs.wiremix
    ] ++ lib.optionals bluetoothEnabled [ pkgs.bluetui ]
    ...
    text = ''
      ...
      case "$panel" in
        ...
        ''}
        *)
          printf 'Użycie: desktop-panel {metrics|audio|wifi${lib.optionalString bluetoothEnabled "|bluetooth"}${lib.optionalString dockerEnabled "|docker"}}\n' >&2
          exit 2
          ;;
      esac
    '';
