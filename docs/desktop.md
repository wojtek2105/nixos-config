...
niego wersjonowany plik `home/base/hyprland.lua`. Konfiguracja korzysta z
...
- Gdy manifest ma `features.voxtype = true`: Voxtype z lokalnym Whisper `large-v3`, polskie dyktowanie uruchamiane na
  ...
  transkrypcji i wynik, a jego fala audio OSD pojawia się podczas nagrywania.
  Ironbar pokazuje jego stan przez zdarzeniowy strumień zgodny z integracją
  Waybara; kliknięcie ikony przełącza nagrywanie. Wbudowany hotkey Voxtype ustawia się w jego
...
prowadzi Plymouth z własnym motywem Biscuit. Na środku jest duża, 720-pikselowa
maskotka Tux z białym brzuchem na przezroczystym tle, trzymająca dwie lustrzane
połówki klawiatury TOTEM; pod nią płynnie obraca się drobny pierścień Biscuit z
różowym łukiem, z częstotliwością około 30 FPS. Tło jest czyste `#000000`, aby na OLED
...
- **Przechwytywanie** — screenshot, podgląd karty HDMI/USB oraz, gdy włączono
  ...
  przełączanie bufora;
- **Narzędzia** — historia schowka oraz, gdy włączono `features.voxtype`,
  przełącznik polskiego dyktowania Voxtype, jego diagnostyka i konfigurator TUI
  w Foot. Ten sam konfigurator jest wtedy widoczny jako aplikacja **Voxtype
  Configure** w launcherze `Super+Space`;
...
`home/base/theme.nix` jest źródłem kolorów, fontów, tapety, motywu GTK, ikon i
...
[wallpapers/CONCEPTS.md](../home/base/wallpapers/CONCEPTS.md), natomiast
[wallpapers/INVENTORY.md](../home/base/wallpapers/INVENTORY.md) przechowuje
...
- sieć, jasność, głośność, bateria, workspaces, tray, zegar oraz — gdy host ma
  `features.bluetooth = true` — Bluetooth używają natywnych modułów Ironbara;
...
- przycisk aparatu pozwala krótkim lewym kliknięciem wskazać całe okno, a
  ...
  a środkowy klik przełącza trwały tryb edycji Satty. Pełne menu z tym samym
  checkboxem jest dostępne pod `Super+Shift+S`; domyślnie każdy zrzut zapisuje
  PNG i kopiuje go do schowka bez otwierania edytora,
- obszar wybiera lekki Slurp, a Wayshot przechwytuje zaznaczenie albo geometrię
  aktywnego okna; po wyborze skrypt czeka jedną klatkę na zniknięcie warstwy
  Slurpa; szybki tryb zapisuje PNG bezpośrednio przez Wayshot. Po włączeniu
  edycji Wayshot zapisuje tymczasowy PNG dla Satty, ponieważ obecna wersja
  edytora nie dekoduje PPM ani QOI. GTK ma wyłączone sondowanie Vulkan, a
  OpenGL wskazuje GPU podłączonego panelu eDP, aby nie budzić dGPU; Enter
  zapisuje wynik, zamyka edytor, a skrypt kopiuje dokładnie jeden zapisany PNG
  do schowka,
- `Print` przechwytuje okno lub obszar, `Shift+Print` aktywne okno, a
  `Ctrl+Print` cały ekran; domyślnie wszystkie zapisują PNG i kopiują go do
  schowka, a po włączeniu trybu edycji otwierają Satty,
...
Polityki można sprawdzić w Zen pod `about:policies`. Moduł `home/base/zen.nix`
...
- ekran blokady Hyprlock zachowuje czysty formularz hasła; nad zegarem pokazuje
  pełną, oryginalną maskotkę Tux + TOTEM o wielkości 372 px (trzykrotnie
  większej niż poprzednio), bez okrągłego kadrowania ani ramki; jest
  zrasteryzowana podczas budowania konfiguracji, więc odblokowanie nie uruchamia
  dodatkowego konwertera ani usługi,
